...
// take last item from an array
// concatinate that item into callback function
...
function firstNamesAllCaps(runners) {
  /* CODE HERE */
  const caps = runners.map(function(toCaps){
...
*/
function getRunnersByTShirtSize(runners, someThing){
  const runnerShirt = runners.filter(function(tShirtSize){
  return tShirtSize.shirt_size == someThing;
  });
  return runnerShirt
};
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  const sums = runners.reduce(function(accu, currVal){
    return accu + currVal.donation;
  }, 0);
return sums;
}
